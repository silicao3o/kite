...
### 15. `com.lite_k8s.node` (멀티 노드 관리) — Phase 6

SSH 터널링을 통한 분산 Docker 노드 관리 및 컨테이너 스케줄링

| 클래스 | 역할 |
|--------|------|
| `Node` | Docker 호스트/노드 상태 및 메트릭 표현 |
| `NodeRegistry` | 전체 관리 노드 중앙 레지스트리 (스레드 안전) |
| `NodeConfig` | 노드 설정 (호스트, 포트, SSH 자격증명) |
| `NodeProperties` | 노드 관리 설정 프로퍼티 |
| `NodeDockerClientFactory` | 원격 노드용 Docker 클라이언트 생성 |
| `SshTunnelManager` | 원격 노드 SSH 터널 관리 |
...
| `NodeConnectionType` (enum) | LOCAL_SOCKET, SSH_TUNNEL, HTTP |
