...
- **사이드카 보호 가드 (`ImageMatchPolicy`)**: 컨테이너 이름 패턴(`quvi*`)이 nginx 사이드카(`quvi-nginx`)까지 잡아도, compose YAML 의 declared image 와 watch image 가 다르면 업데이트 차단. `kite.service-definition-id` 라벨 → ServiceDefinition → compose YAML → service 의 image 를 진실의 소스로 사용. 라벨이 없는 컨테이너는 runtime image short-name 비교로 fallback.
- **이미지 retention (`ImageRetentionService`)**: recreate 직후 같은 repo 의 dangling 이미지 정리. 사용중 (running + stopped 컨테이너 참조) 은 절대 안 지움, 나머지는 created 기준 최신 K개 (default 3, `kite.image-retention.keep-recent`) 유지. 매 watch 마다 새 digest 가 누적돼 daemon 의 `no space left on device` 로 다음 pull 이 실패하던 문제 방지.
...
- **노드별 Env Profile 매핑**: 같은 Compose를 여러 노드에 서로 다른 Env Profile로 배포 가능
  - `nodeEnvMappings`: `{"node-01": "profile-prod", "node-02": "profile-staging"}` 형태로 노드별 환경 설정
  - 배포 시 각 노드별 매핑된 Env Profile의 환경변수를 자동 주입
  - UI에서 노드 추가 버튼으로 동적으로 노드-프로파일 매핑 행 추가
  - 하위호환: 기존 `envProfileId` + `nodeNames` 형식도 자동 변환
...
| Service Definitions | ComposeParser profiles 파싱/필터링, ServiceDeployer, ServiceDefinition 엔티티, 노드별 Env Profile 매핑, StringMapConverter | 42 |
