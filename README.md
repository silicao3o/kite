...
- **노드 등록**: TCP 직접 연결 및 SSH 터널 연결 지원
...
- **SSH 터널**: JSch 기반 SSH 터널로 온프레미스 서버 연결 (TCP 오픈 불필요)
- **원격 메트릭 수집**: 원격 노드 컨테이너 CPU/메모리 15초마다 수집
- **UI 노드 등록**: /nodes 페이지에서 TCP/SSH 노드 동적 등록

### SSH_PROXY — CP 경유 연결 (Phase 7.1-C)
- **3종 연결 방식**: TCP(직접) / SSH(직접 터널) / SSH_PROXY(CP 점프 호스트 경유)
...
### 이미지 업데이트 & 컨테이너 재생성
- **최신 이미지 Pull**: 컨테이너 탭 및 상세 탭의 **Update** 버튼으로 최신 이미지를 즉시 Pull
- **무설정 재생성**: compose 파일 없이도 기존 컨테이너 설정(env, ports, volumes, labels)을 그대로 읽어 재생성
- **자동 교체 흐름**: pull → stop → remove → create → start 순서로 자동 실행
- **멀티노드 지원**: nodeId 파라미터로 원격 노드 컨테이너도 업데이트 가능
- **REST API**: `POST /api/containers/{id}/update-image?nodeId={nodeId}`
...
| Desired State 관리 | CrashLoopBackoffTracker, StateReconciler | 11 |
...
| 이미지 업데이트 & 재생성 | ContainerRecreateService (이미지 추출, pull, config 빌드, 재생성 순서, 멀티노드), ContainerController | 8 |
