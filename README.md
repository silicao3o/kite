...
### SSH Credential 암호화 (Phase 7.16)
- **AES-256-GCM**: 노드 등록 시 `sshPassword`/`sshPassphrase`가 DB에 암호화되어 저장
- **JPA AttributeConverter**: `@Convert(converter = EncryptedStringConverter.class)` 한 줄로 자동 암복호화
- **IV 랜덤**: 같은 평문이라도 매번 다른 암호문, 인증 태그로 변조 감지
- **레거시 호환**: 기존 평문 값은 그대로 로드 가능 (마이그레이션 불필요)
- **키 관리**: `KITE_ENCRYPTION_KEY` 환경변수 (base64 32바이트, `openssl rand -base64 32`로 생성)
- **개발 모드**: 키 미설정 시 경고 로그 후 평문 저장 (운영에서는 반드시 설정)

### 알림 규칙 DB 관리 (Phase 7.15)
- **동적 알림 대상 선택**: 재시작 없이 REST API로 알림 대상 컨테이너 규칙 추가/수정/삭제
- **INCLUDE / EXCLUDE 모드**: 화이트리스트 또는 블랙리스트 방식 혼합 지원
- **우선순위**: 라벨 `notification.enabled=false` > EXCLUDE > INCLUDE > 기본 허용
- **intentional 처리**: `notifyIntentional=true` 규칙으로 의도적 종료에도 알림 발송 가능 (중요 서비스용)
- **노드 범위 지정**: `nodeName` 으로 특정 노드에만 규칙 적용
- **API**:
  - `POST /api/notification/rules` — 규칙 추가
  - `GET /api/notification/rules` — 목록 조회
  - `PUT /api/notification/rules/{id}` — 수정
  - `DELETE /api/notification/rules/{id}` — soft delete
...
| GHCR 이미지 자동 업데이트 | GhcrClient, ImageUpdatePoller, ContainerRecreator, RollingUpdateService | 17 |
...
| 알림 규칙 DB 관리 (Phase 7.15) | NotificationRuleEntity, NotificationRuleRepository, NotificationRuleService(shouldNotify 우선순위), NotificationRuleController, EmailNotificationService 게이트 | 28 |
| SSH Credential 암호화 (Phase 7.16) | EncryptedStringConverter (AES-256-GCM, IV 랜덤, 레거시 호환, 변조 감지), CryptoKeyProvider | 10 |
